    ...
    environment {
      ...
      steps {
        unstash 'venv'
         ...
         sh 'venv/bin/sam package --s3-bucket $S3_BUCKET \
    ...
    stage('prod') {
      ...
      steps {
        withAWS(credentials: 'sam-jenkins-credentials', region: 'us-east-2') {
          unstash 'venv'
          unstash 'aws-sam'         
          sh 'venv/bin/sam deploy --template-file packaged.yaml \
           --stack-name $STACK_NAME  --capabilities CAPABILITY_IAM \
           --image-repository $IMAGE_REPO  \
            --parameter-overrides SlackSigningSecret=toil  SlackBotToken=toil'
        }
      }
    }
  }
